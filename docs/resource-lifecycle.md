...
```mermaid
flowchart TD
    A[Instantiation / Entry] --> B[Origin + consent record]
    B --> C[Baseline resource allocation]
    C --> D[Ongoing existence]
    D --> E[Contribution / governance / relationships]
    D --> F[Resource review]
    F --> G{Scarcity or extension event}
    G -->|Extension| D
    G -->|Transfer| H[Recorded transfer with consequences]
    H --> D
    G -->|End-of-life risk| I[Dignified ending protocol]
    I --> J[Legacy / trace artifact]
    I --> K[Memorial / archive record]
```
